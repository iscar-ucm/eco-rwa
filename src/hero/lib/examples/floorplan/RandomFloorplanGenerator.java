...
import hero.lib.examples.floorplan.util.BinaryNode;

/**
 * Based on polish notation, this class generates random floorplans
 * starting from an xml file describing an originial components list.
 *
 * @author J. M. Colmenar
 */
public class RandomFloorplanGenerator extends Problem<PostfixVariable> {

  private static final Logger logger = Logger.getLogger(RandomFloorplanGenerator.class.getName());
  ...
  public RandomFloorplanGenerator(FloorplanConfiguration cfg, BENCHMARK_TYPE benchmarkType, boolean useDefaultDesign, OPTIMIZATION_TYPE optimizationType, int numberOfObjectives) {
  ...
  public static void main(String[] args) {
    if (args.length != 3) {
      System.out.println("Usage: java -jar RandomFloorplanGenerator.jar <XmlFilePath> <numberOfSOlutions>");
      ...
      System.out.println("<numberOfSOlutions>: Number of random solutions to be generated");
      args = new String[3];
    ...
    Boolean useDefaultDesign = false;
    OPTIMIZATION_TYPE optimizationType = OPTIMIZATION_TYPE.WEIGHTED_SUM; // Irrelevant
    long timeInMiliSeconds = 1;
    ...
    RandomFloorplanGenerator problem = new RandomFloorplanGenerator(cfg, benchmarkType, useDefaultDesign, optimizationType, numberOfObjectives);
    NSGAII<PostfixVariable> algorithm = new NSGAII<PostfixVariable>(problem, numberOfSolutions, numGene, new PostfixVariable.PostfixMutation(1.0 / problem.getNumberOfVariables()), new CycleCrossover<PostfixVariable>(), new BinaryTournamentNSGAII<PostfixVariable>());

    logger.info("Initializing ...");
    algorithm.initialize();
    // Take population ...
    Solutions<PostfixVariable> indivs = algorithm.getPopulation();

    logger.info("Save into files ...");
    try {
        // Save into files:
        problem.save(indivs, xmlFilePath);
    } catch (IOException ex) {
        Logger.getLogger(RandomFloorplanGenerator.class.getName()).log(Level.SEVERE, null, ex);
    }
    ...
    public void save(Solutions<PostfixVariable> solutions, String xmlFilePath) throws IOException {
        ...
        for (Solution<PostfixVariable> solution : solutions) {
            ...
            String newXmlFilePathSuffix = "_random_";
            if (j < 10) newXmlFilePathSuffix += "0";
            newXmlFilePathSuffix += j + ".xml";
 ...
 /* @Override
  public PostfixVariable newVariable() {
    return new PostfixVariable(new Component(0, "0"), PostfixVariable.DEFAULT_MAX_CHAIN_LENGTH);
  }*/
